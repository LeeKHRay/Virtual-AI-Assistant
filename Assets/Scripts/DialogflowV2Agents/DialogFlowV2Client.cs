...

{
	...
	public class DialogFlowV2Client : MonoBehaviour 
	{
		...
		public delegate void RetryFailErrorHandler();

		public event RetryFailErrorHandler RetryFailError;
		...
		/// <summary>
		/// Delegate for handling responses from speech recognition. 
		/// </summary>
		public delegate void SpeechRecognitionResponseHandler(DF2Response response);

		/// <summary>
		/// Event fired at each response from speech recognition
		/// </summary>
		public event SpeechRecognitionResponseHandler SpeechRecognitionResponse;
		...
		public void DetectIntentFromText(string text, string languageCode = "")
		{
			...
			StartCoroutine(DetectIntent(queryInput, sessionID));
		}
		...
		public void DetectIntentFromAudio(string audio64, string languageCode = "")
		{
			...
			queryInput.AudioConfig.LanguageCode = "zh-HK"; //languageCode;			
			StartCoroutine(DetectIntent(queryInput, sessionID, audio: audio64));
		}
		...
		/// <param name="languageCode">The language code of the request.</param>
		public void DetectIntentFromEvent(string eventName, Dictionary<string, object> parameters, Dictionary<string, float> outputAudioConf = null, string languageCode = "")
		{
			if (languageCode.Length == 0)
				languageCode = accessSettings.LanguageCode;
			...
			StartCoroutine(DetectIntent(queryInput, sessionID, outputAudioConf));
		}
		...
		/// <param name="languageCode">The language code of the request.</param>
		public void DetectIntentFromGenericInput(DF2QueryInput input, string languageCode = "")
		{
			if (languageCode.Length == 0)
				languageCode = accessSettings.LanguageCode;

			StartCoroutine(DetectIntent(input, sessionID));
		}
		...
		private IEnumerator DetectIntent(DF2QueryInput queryInput, string session, Dictionary<string, float> outputAudioConf = null, string audio = "")
		{
			// Gets the JWT access token.
			string accessToken = string.Empty;
			while (!JwtCache.TryGetToken(accessSettings.ServiceAccount, out accessToken))
				yield return JwtCache.GetToken(accessSettings.CredentialsFileName, accessSettings.ServiceAccount);
			...
			// Configure output audio setting
			...
			request.OutputAudioConfig.SynthesizeSpeechConfig.Voice.Name = "en-US-Wavenet-F";
			if(outputAudioConf != null)
			{
				if (outputAudioConf.ContainsKey("speakingRate"))
					request.OutputAudioConfig.SynthesizeSpeechConfig.SpeakingRate = outputAudioConf["speakingRate"];
				if (outputAudioConf.ContainsKey("pitch"))
					request.OutputAudioConfig.SynthesizeSpeechConfig.Pitch = outputAudioConf["pitch"];
				if (outputAudioConf.ContainsKey("volumeGainDb"))
					request.OutputAudioConfig.SynthesizeSpeechConfig.VolumeGainDb = outputAudioConf["volumeGainDb"];
			}
			...
			{
				DF2ErrorResponse dF2ErrorResponse = JsonConvert.DeserializeObject<DF2ErrorResponse>(df2Request.downloadHandler.text);
				DetectIntentError?.Invoke(dF2ErrorResponse);

				if(dF2ErrorResponse.error.code == 503 || dF2ErrorResponse.error.code == 504)
				{
					if (retryNum < 2) // retry at most 2 times
					{
						retryNum++;
						Debug.Log("retryNum: " + retryNum);
						int waitTime = new System.Random().Next(1, (int)Math.Pow(2, retryNum));
						yield return new WaitForSeconds(waitTime);

						StartCoroutine(DetectIntent(queryInput, session, outputAudioConf, audio));
					}
					else
					{
						RetryFailError?.Invoke();
						retryNum = 0;
					}
				}
			}
			else
			{
				...
				{
					foreach (DF2Context context in resp.queryResult.outputContexts)
					{
						AddInputContext(context, "1234");
					}
				}
				...
				if (audio == "")
					...
					SpeechRecognitionResponse?.Invoke(resp);
		...
		{
			StartCoroutine(ClearSessionRequest(sessionID));
		}
