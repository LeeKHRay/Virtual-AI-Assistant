...
public class TetrisGameboard : MonoBehaviour
{
    ...
    public AudioClip clearLine1;
    public AudioClip clearLine2;

    public Transform holdText;
    public Transform holdTextPos;
    public Transform heldTetromino;
    public Transform holdPos;
    private SpriteRenderer[] heldTetrominoSr;
    public Transform scoreText;
    public Transform scoreTextPos;
    public Transform score;
    public Transform scorePos;
    public Transform attackGaugePos;
    public AttackGauge attackGauge;
    public GameObject warning;
    public Transform warningPos;
    public TetrisGameboard opponentGameboard;
    ...
    void Start()
    {
        ...
        if (isBattle)
        {
            // align positions of attackGauge and warning
            attackGauge.transform.position = attackGaugePos.position;
            warning.transform.position = warningPos.position;
            warningText = warning.GetComponentInChildren<TMP_Text>();
        }
		else {
            // align positions of scoreText and score
			scoreText.position = scoreTextPos.position;
			score.position = scorePos.position;
		}
    ...
    void Update()
    {
        heldTetromino.position = holdPos.position + heldTetromino.position - heldTetromino.GetChild(4).position;
		holdText.position = holdTextPos.position;

        if (isBattle)
        {
            if (garbageRowsNum > 0)
            {
                // show warning under gameboard
                warning.SetActive(true);
                warningText.text = garbageRowsNum + (garbageRowsNum == 1 ? " row is" : " rows are") + " coming!";
            }
            else
            {
    ...
    public float SpecialEffect(List<int> fullRows)
    {
        float waitTime = 0.0f;
        foreach (int y in fullRows)
        {
            for (int x = 0; x < width; x++)
            {
                waitTime = grid[x, y].gameObject.GetComponent<Block>().Clear(fullRows.Count);
            }
        }
        return waitTime;
    }
    ...
    public int RemoveFullRows()
    {
        ...
        int fullRowsNum = fullRows.Count;
        StartCoroutine(RemoveRows(fullRows));
        if (fullRowsNum > 0)
        {
            if (isBattle)
            {
                opponentGameboard.garbageRowsNum += attackGauge.UpdateAttackGauge(fullRowsNum);
            }
            else
            {
                // update score
                if (isPlayer)
                {
                    TetrisSceneManager.playerScore += fullRowsNum * fullRowsNum;
                }
                else
                {
                    TetrisSceneManager.aiScore += fullRowsNum * fullRowsNum;
                }
            }
        }
    ...
    private IEnumerator RemoveRows(List<int> fullRows)
    {
        int fullRowsNum = fullRows.Count;
        float waitTime = SpecialEffect(fullRows);

        if (fullRowsNum == 2 || fullRowsNum == 3)
        {
            SetLightning(fullRows, true); // create lightning effect
        }
        ...
        if (fullRowsNum == 2 || fullRowsNum == 3)
        {
            SetLightning(fullRows, false); // hide lightning
        }

        int droppedRowsNum = 0;
        foreach (int row in fullRows)
        {
            DropRows(row - droppedRowsNum + 1);
            droppedRowsNum++;
        }
        UpdateHeights();
    ...
    private void SetLightning(List<int> fullRows, bool trigger)
    {
        int lightningNum = fullRows.Count;
        LightningBoltScript[] lightning = lightningNum == 2 ? lightning1 : lightning2;

        for (int i = 0; i < lightningNum; i++)
        {
            lightning[i].transform.position += trigger ? new Vector3(0, fullRows[i], 0) : new Vector3(0, -fullRows[i], 0);
            lightning[i].lineRenderer.enabled = trigger;
            lightning[i].trigger = trigger;
        }
    }

    public void RiseRows()
    {
        for (int i = 0; i < garbageRowsNum; i++)
        {
    ...
    private void CreateGarbage()
    {
        for (int i = 0; i < garbageRowsNum; i++)
        {
            GameObject garbage = spawner.SpawnGarbage(i);
            foreach (Transform child in garbage.transform)
            {
                Vector2 pos = RoundVec2(child.position - posOffset);
            ...
            for (int i = 0; i < rotationNum[shape]; i++)
            {
                ...
                while (!IsRightMost(tetromino, blockPos[i], x))
                {
                    ...
                    if (fitness > bestFitness)
                    {
                        ...
                        bestDir = Quaternion.Euler(0, 0, -90 * i) * tetrominoesDir[idx];
    ...
    public void SoundEffect(string soundEffectName)
    {
        ...
        {
            case "drop":
                soundEffect.clip = drop;
                break;
            case "rotate":
                soundEffect.clip = rotate;
                break;
            case "swap":
                soundEffect.clip = swap;
                break;
            case "clearLine1":
                soundEffect.clip = clearLine1;
                break;
            case "clearLine2":
                soundEffect.clip = clearLine2;
                break;
        }
        soundEffect.Play();
    }
